...
const { businessArr } = require("../../yelp_dataset/business_dataset.js");
...
async function main() {
  ...
  // console.log(`${((i / businessArr.length) * 100).toFixed(2)}%`);
  // const businessExists = await prisma.business.findUnique({
  // where: {
  // id: businessArr[i].business_id,
  // },
  // });
  ...
  // check if business records exist & skip if they do
  // const check = await prisma.categoryToBusiness.findMany({
  // where: {
  // businessId: businessArr[i].business_id,
  // },
  // });
  // if (check.length > 0) {
  // continue;
  ...
  // const splitCategories = businessArr.categories.split(", ");
...
main()
  ...
  .then(() =>
    console.log("Categories to Businesses seeded. Client Disconnected")
  )
  .catch(async (e) => {
    console.log(e);
    await prisma.$disconnect();
